...
import './css/home-horizontal-slider.css';
...
const getItems = () =>
  data.map((car, index) => ({ id: index, logo: car.logo, name: car.name }));

function HomeHorizontalSlider() {
  ...
  return (
    <ScrollMenu LeftArrow={LeftArrow} RightArrow={RightArrow}>
      {items.map(({ logo,name,id }) => (
        <Card
          className="scroll__item"
          itemId={id}
          logo={logo} // NOTE: itemId is required for track items
          title={name}
          key={id}
...
function RightArrow() {
  ...
  return (
    // eslint-disable-next-line react/button-has-type
    <button
      className="scroll__button button__right"
...
function Card({ onClick, selected, title, logo, itemId }) {
